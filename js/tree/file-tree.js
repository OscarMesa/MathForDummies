    ...
    $.fn.fileTree = function(options){
        ...
        function _bindListeners(obj){
            $(obj).find('.tree li.folder > div').on('click', function() {
                if($(this).find('i').first().attr('class') == 'icon-folder-close'){
                    $(this).find('i').first().addClass('icon-folder-open')
                    $(this).find('i').first().removeClass('icon-folder-close');
                }else{
                    $(this).find('i').first().removeClass('icon-folder-open')
                    $(this).find('i').first().addClass('icon-folder-close');
                }
        ...
        function _renderFile(file){
            ...
            if(file.url !== undefined && options.selectable !== true){
                listItem = listItem + '<a href="' + file.url + '"><i class="icon-file"></i>' + file.name + '</a></div></li>';
            }else{
                listItem = listItem + '<i class="icon-file"></i>' + file.name + '</div></li>';
            }
        ...
        function _renderDir(dir){
            var listItem;
            if(dir.id !== undefined){
                listItem = '<li class="folder" data-id="' + dir.id + '" data-type="folder" data-name="' + dir.name + '" ><div><i class="icon-folder-close"></i>' + dir.name + '</div>';
            }else{
                listItem = '<li class="folder"><div><i class="icon-folder-open"></i>' + dir.name + '</div>';
            }
