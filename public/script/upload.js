$(document).ready(function() {
	...
	*****************************/
	var imgWidth = 180,
		...
		files = undefined, 
		type_video =  {'video/webm':'','video/ogg':'','video/mp4':''};
	...
	*****************************/
	...
	dropzone.on('drop', function(e) {
		...
		if( (/video/i).test(files[0].type) ) {
			if(type_video.hasOwnProperty(files[0].type) || select_server.val() != 'math')
			{
				dropzone.removeClass('hover');
				...
				$('#video-acepted').css('display','block');
				$('.fileupload-exists').css('display','inline-block');
				$('#title-video').css('display','inline-block');
				$('#title-video').html(files[0].name);
			}else{
				OpenMessagesErrorModal("Error","Este servidor solo soporta videos en formato webm, ogg y mp4");
				dropzone.removeClass('hover');
			}
		} else {
			//some message for wrong file format
			OpenMessagesErrorModal("Error","Solo se soportan videos.");
		}
	...
	defaultUploadBtn.on('change', function(e) {
		...
		{
			if( (/video/i).test($(this)[0].files[0].type) ) {
				files = $(this)[0].files;
				if(type_video.hasOwnProperty($(this)[0].files[0].type) || select_server.val() != 'math')
				{
					//processFiles(files);
					$('#droparea').css('display','none');
					$('#video-acepted').css('display','block');
